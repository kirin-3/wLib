...
<template>
  <div class="p-8 max-w-4xl pb-12">
    <header class="mb-8">
      <h2 class="text-3xl font-bold mb-2 tracking-tight" style="color: var(--text-primary)">Updates</h2>
      <p class="text-sm pl-3" style="color: var(--text-secondary); border-left: 2px solid var(--brand)">Check F95Zone for new game versions.</p>
    </header>

    <div class="space-y-6">

      <!-- App Update Available (GitHub) -->
      <section v-if="appUpdate" class="rounded-xl p-6 shadow-lg relative overflow-hidden" style="background: linear-gradient(135deg, var(--bg-surface), var(--bg-inset)); border: 1px solid var(--brand-deep)">
        <div class="absolute top-0 right-0 w-64 h-64 rounded-full blur-3xl -mr-20 -mt-20 pointer-events-none" style="background: var(--brand-glow)"></div>
        
        <div class="flex items-start justify-between mb-4 relative z-10">
          <div>
            <h3 class="text-xl font-bold flex items-center gap-2 mb-1" style="color: var(--text-primary)">
              <svg class="w-6 h-6" style="color: var(--brand)" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>
              wLib Update Available
            </h3>
            <div class="flex items-center gap-3">
              <span class="text-xs font-mono px-2 py-0.5 rounded-md" style="background: var(--bg-raised); color: var(--text-muted); border: 1px solid var(--border)">Current: v0.1</span>
              <span class="text-xs" style="color: var(--text-muted)">→</span>
              <span class="text-xs font-mono px-2 py-0.5 rounded-md font-bold animate-pulse" style="background: var(--brand-glow); color: var(--brand); border: 1px solid var(--brand-deep)">New: {{ appUpdate.version }}</span>
            </div>
          </div>
          
          <div class="flex gap-2 shrink-0">
             <button @click="openInBrowser(appUpdate.url)" class="update-btn flex items-center gap-1.5">
        ...
        <div class="changelog-wrapper rounded-lg p-5 text-sm relative z-10 max-h-64 overflow-y-auto wlib-changelog" style="color: var(--text-secondary)" v-html="appUpdate.changelogHtml"></div>
      </section>
      ...
      <section class="section-card rounded-xl p-6">
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-lg font-semibold flex items-center gap-2" style="color: var(--text-primary)">
            <svg class="w-5 h-5" style="color: var(--brand)" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            Automatic Checking
          </h3>
        </div>
        ...
        <div class="flex items-center gap-3 mb-3">
          ...
          <div class="flex items-center rounded-lg overflow-hidden" style="border: 1px solid var(--border)">
            <button v-for="opt in [{key:'weekly',label:'Weekly'},{key:'monthly',label:'Monthly'},{key:'off',label:'Off'}]" :key="opt.key"
              ...
              :style="autoCheckFreq === opt.key ? 'background: var(--bg-overlay); color: var(--text-primary)' : 'background: var(--bg-surface); color: var(--text-muted)'">
              {{ opt.label }}
            </button>
          </div>
        </div>
        <p class="text-xs" style="color: var(--text-muted)">Last checked: {{ formatLastCheck }}</p>
      </section>
      ...
      <section class="section-card rounded-xl p-6">
        ...
        <p v-if="!status.running && status.total === 0" class="text-sm" style="color: var(--text-muted)">
      ...
      <!-- Updates Available -->
      <section v-if="persistedUpdates.length > 0" class="section-card rounded-xl p-6" style="border-color: rgba(34, 197, 94, 0.2)">
        ...
        <TransitionGroup name="list" tag="div" class="space-y-2">
          <div v-for="game in persistedUpdates" :key="game.id"
            ...
            <div class="flex items-center gap-3 min-w-0">
              ...
              <div class="w-6 h-6 rounded shrink-0 flex items-center justify-center" style="background: var(--bg-overlay)" v-else>
                <svg class="w-3 h-3" style="color: var(--text-muted)" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/></svg>
              </div>
              <div class="min-w-0">
                <p class="text-sm font-semibold truncate" style="color: var(--text-primary)">{{ game.title }}</p>
                <p class="text-xs" style="color: var(--text-muted)">{{ game.developer }}</p>
              </div>
            </div>
            <div class="flex items-center gap-3 shrink-0">
              <div class="text-right">
                <span class="text-xs font-mono" style="color: var(--text-muted)">{{ game.version }}</span>
                <span class="text-xs mx-1" style="color: var(--text-muted)">→</span>
              ...
              <button v-if="game.f95_url" @click="openInBrowser(game.f95_url)"
                class="p-1.5 rounded-lg transition-all" style="color: #b380cc" title="Open F95Zone thread">
      ...
      <!-- Check Results -->
      <section v-if="!status.running && status.results.length > 0" class="section-card rounded-xl p-6">
        <h3 class="text-lg font-semibold mb-4 flex items-center gap-2" style="color: var(--text-primary)">
          <svg class="w-5 h-5" style="color: var(--brand)" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" x2="8" y1="13" y2="13"/><line x1="16" x2="8" y1="17" y2="17"/></svg>
          Last Check Results
        </h3>

        <div class="grid grid-cols-3 gap-4 mb-4">
          <div class="rounded-lg p-3 text-center" style="background: var(--bg-raised); border: 1px solid var(--border)">
            <p class="text-2xl font-bold" style="color: var(--text-primary)">{{ gamesChecked.length }}</p>
            <p class="text-xs" style="color: var(--text-muted)">Checked</p>
          </div>
          <div class="rounded-lg p-3 text-center" style="background: var(--bg-raised); border: 1px solid rgba(34, 197, 94, 0.2)">
            <p class="text-2xl font-bold text-green-400">{{ gamesWithUpdates.length }}</p>
            <p class="text-xs" style="color: var(--text-muted)">Updates</p>
          </div>
          <div class="rounded-lg p-3 text-center" style="background: var(--bg-raised); border: 1px solid rgba(239, 68, 68, 0.2)">
            <p class="text-2xl font-bold" :class="gamesWithErrors.length ? 'text-red-400' : ''" :style="!gamesWithErrors.length ? 'color: var(--text-muted)' : ''">{{ gamesWithErrors.length }}</p>
            <p class="text-xs" style="color: var(--text-muted)">Errors</p>
          </div>
        </div>
        
        <div v-if="gamesWithErrors.length" class="space-y-1">
          <p class="text-xs mb-1" style="color: var(--text-muted)">Failed checks:</p>
...
:deep(.wlib-changelog h2) { font-size: 1.1rem; border-bottom: 1px solid var(--border); padding-bottom: 0.25rem; }
...
:deep(.wlib-changelog a) { color: var(--brand); text-decoration: underline; text-underline-offset: 2px; }
:deep(.wlib-changelog a:hover) { filter: brightness(1.2); }
:deep(.wlib-changelog code) { background: var(--bg-raised); padding: 0.1rem 0.3rem; border-radius: 4px; font-family: monospace; font-size: 0.85em; }
:deep(.wlib-changelog pre) { background: var(--bg-inset); padding: 1rem; border-radius: 8px; overflow-x: auto; margin-bottom: 1rem; border: 1px solid var(--border); }
