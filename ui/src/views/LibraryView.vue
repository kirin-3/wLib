...
const activeFilterCount = computed(() => filterStatuses.value.length + filterEngines.value.length + filterTags.value.length)
const clearFilters = () => { filterStatuses.value = []; filterEngines.value = []; filterTags.value = [] }
...
const filteredGames = computed(() => {
    ...
    if (filterTags.value.length) {
        result = result.filter(g => {
            const gameTags = typeof g.tags === 'string' ? g.tags.split(',').map(t => t.trim()) : (g.tags || [])
...
<template>
  <div class="p-8 h-full flex flex-col">
    ...
    <header class="flex justify-between items-center mb-8">
      <div>
        <h2 class="text-3xl font-bold mb-2 tracking-tight" style="color: var(--text-primary)">Your Library</h2>
        <p class="text-sm pl-3" style="color: var(--text-secondary); border-left: 2px solid var(--brand)">Manage and play your imported games.</p>
      </div>
    ...
    <div class="mb-6 space-y-3">
      <!-- Search Input -->
      <div class="relative">
        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style="color: var(--text-muted)" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
      ...
      <div class="flex items-center gap-3">
        <button @click="showFilters = !showFilters"
          ...
          :style="showFilters || activeFilterCount > 0 ? 'background: var(--brand-glow); border: 1px solid var(--brand-deep); color: var(--brand)' : 'background: var(--bg-surface); border: 1px solid var(--border); color: var(--text-secondary)'">
          ...
          Filters
          <span v-if="activeFilterCount" class="text-white rounded-full w-4 h-4 text-[10px] flex items-center justify-center" style="background: var(--brand)">{{ activeFilterCount }}</span>
        </button>
        
        <!-- Sort Buttons -->
        <div class="flex items-center rounded-lg overflow-hidden" style="border: 1px solid var(--border)">
          <button v-for="s in [{key:'title',label:'A-Z'},{key:'f95_rating',label:'F95 ★'},{key:'own_rating',label:'My ★'}]" :key="s.key"
            ...
            :style="sortBy === s.key ? 'background: var(--bg-overlay); color: var(--text-primary)' : 'background: var(--bg-surface); color: var(--text-muted)'">
            {{ s.label }}
        ...
        <button v-if="activeFilterCount > 0" @click="clearFilters"
          class="text-xs hover:text-red-400 transition-colors flex items-center gap-1" style="color: var(--text-muted)">
          <svg class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12"/></svg>
          Clear all
        </button>
        
        <div class="ml-auto flex items-center gap-4">
          <span class="text-xs" style="color: var(--text-muted)">{{ filteredGames.length }} game{{ filteredGames.length !== 1 ? 's' : '' }}</span>
          
          <!-- Layout Toggle -->
          <div class="flex items-center rounded-lg p-1" style="background: var(--bg-surface); border: 1px solid var(--border)">
            <button @click="layoutMode = 'grid'" 
              class="p-1.5 rounded-md transition-all"
              :style="layoutMode === 'grid' ? 'background: var(--bg-overlay); color: var(--text-primary); box-shadow: 0 1px 2px rgba(0,0,0,0.1)' : 'color: var(--text-muted)'"
              title="Grid View">
            ...
            <button @click="layoutMode = 'list'" 
              class="p-1.5 rounded-md transition-all"
              :style="layoutMode === 'list' ? 'background: var(--bg-overlay); color: var(--text-primary); box-shadow: 0 1px 2px rgba(0,0,0,0.1)' : 'color: var(--text-muted)'"
              title="List View">
      ...
      <div v-if="showFilters" class="space-y-3 pt-2">
        ...
        <div class="flex flex-wrap items-center gap-1.5">
          <span class="text-[10px] uppercase tracking-wider font-semibold w-14 shrink-0" style="color: var(--text-muted)">Status</span>
          <button v-for="s in allStatuses" :key="s.value" @click="toggleFilter(filterStatuses, s.value)"
            ...
            :style="filterStatuses.includes(s.value) ? 'background: var(--brand-glow); border: 1px solid var(--brand-deep); color: var(--brand)' : 'background: var(--bg-surface); border: 1px solid var(--border); color: var(--text-secondary)'">
            {{ s.label }}
          </button>
        </div>
        ...
        <div v-if="uniqueEngines.length" class="flex flex-wrap items-center gap-1.5">
          <span class="text-[10px] uppercase tracking-wider font-semibold w-14 shrink-0" style="color: var(--text-muted)">Engine</span>
          <button v-for="eng in uniqueEngines" :key="eng" @click="toggleFilter(filterEngines, eng)"
            class="px-2.5 py-1 rounded-full text-xs transition-all"
            :style="filterEngines.includes(eng) ? 'background: rgba(90, 57, 104, 0.2); border: 1px solid var(--brand-deep); color: #b380cc' : 'background: var(--bg-surface); border: 1px solid var(--border); color: var(--text-secondary)'">
            {{ eng }}
          </button>
        </div>
        ...
        <div v-if="uniqueTags.length" class="flex flex-wrap items-center gap-1.5">
          <span class="text-[10px] uppercase tracking-wider font-semibold w-14 shrink-0" style="color: var(--text-muted)">Tags</span>
          <button v-for="tag in uniqueTags" :key="tag" @click="toggleFilter(filterTags, tag)"
            class="px-2.5 py-1 rounded-full text-xs transition-all"
            :style="filterTags.includes(tag) ? 'background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.4); color: #34d399' : 'background: var(--bg-surface); border: 1px solid var(--border); color: var(--text-secondary)'">
            {{ tag }}
          </button>
        </div>
      </div>
    ...
    <TransitionGroup name="list" tag="div" :class="['grid gap-4 md:gap-6 pb-12', layoutMode === 'grid' ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1']">
      ...
      <div v-for="game in filteredGames" :key="game.id" @click="openDetail(game)"
           ...
           :class="layoutMode === 'grid' ? 'flex flex-col lg:flex-row w-full h-auto min-h-[14rem] lg:h-56' : 'flex flex-row items-center w-full h-24 md:h-32 pr-2 md:pr-4'">
        
        <!-- Cover Image -->
        <div :class="[layoutMode === 'grid' ? 'w-full lg:flex-1 h-56 lg:h-full' : 'w-24 md:w-36 lg:w-48 h-full', 'flex items-center justify-center relative shadow-inner overflow-hidden shrink-0']"
             style="background: linear-gradient(135deg, var(--bg-raised), var(--bg-inset))">
          <img v-if="game.cover_image_path" :src="game.cover_image_path" :alt="game.title"
            class="absolute inset-0 w-full h-full object-cover object-top" />
          <svg v-else class="w-12 h-12 transition-colors" style="color: var(--border)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><line x1="3" x2="21" y1="9" y2="9"/><path d="M9 21V9"/></svg>
        ...
        <div :class="['p-4 md:p-5 flex shrink-0 min-w-0', layoutMode === 'grid' ? 'flex-col justify-between w-full lg:w-72 xl:w-80 h-full' : 'flex-1 flex-row items-center justify-between gap-4 h-full']">
          ...
          <div :class="['min-w-0 flex flex-col', layoutMode === 'grid' ? '' : 'justify-center h-full']">
            <h3 :class="['font-bold truncate mb-1', layoutMode === 'grid' ? 'text-lg md:text-xl lg:text-2xl' : 'text-base md:text-xl']" style="color: var(--text-primary)" :title="game.title">{{ game.title }}</h3>
            <p v-if="game.developer" :class="['truncate mb-2 md:mb-3', layoutMode === 'grid' ? 'text-xs md:text-sm' : 'text-[10px] md:text-xs']" style="color: var(--text-muted)">by {{ game.developer }}</p>
            
            <div :class="['flex items-center gap-1.5', layoutMode === 'grid' ? 'mt-2' : '']">
              <div class="flex items-center gap-1.5 px-2 py-0.5 md:py-1 rounded-md" style="background: var(--bg-raised); border: 1px solid var(--border)">
                <span class="font-mono text-[10px] md:text-xs font-bold" style="color: var(--brand)">{{ game.version || 'Unknown' }}</span>
              </div>
          ...
          <div :class="['flex items-center shrink-0', layoutMode === 'grid' ? 'pt-4 justify-between mt-auto' : 'gap-4 md:gap-6']">
            <div v-if="layoutMode === 'grid'" class="text-xs font-medium whitespace-nowrap overflow-hidden text-ellipsis max-w-[50%]" style="color: var(--text-muted)" :title="game.status">
            ...
            <div v-if="layoutMode === 'list'" class="hidden sm:block w-24 text-right text-[10px] font-medium whitespace-nowrap overflow-hidden text-ellipsis" style="color: var(--text-muted)" :title="game.status">
               {{ game.status === 'completed' ? '✅ Done' : game.status === 'in_progress' ? '🎮 Playing' : game.status === 'replaying' ? '🔄 Replay' : game.status === 'waiting_update' ? '⏳ Waiting' : game.status === 'abandoned' ? '🚫' : 'Not Started' }}
            </div>
          </div>
        </div>
