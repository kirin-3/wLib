...
const findSaves = async () => {
    ...
    try {
        const results = await api.findSaveFiles(props.game.exe_path, title.value, engine.value)
...
<template>
  <div v-if="modelValue && game" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <!-- Modal Content -->
    <div class="modal-content w-full max-w-2xl rounded-2xl shadow-2xl relative overflow-hidden transform transition-all flex flex-col max-h-[90vh]">
      
      <!-- Header with Cover Image Background -->
      <div class="relative h-40 overflow-hidden" style="background: linear-gradient(135deg, var(--bg-raised), var(--bg-inset))">
        <img v-if="coverImage" :src="coverImage" class="absolute inset-0 w-full h-full object-cover opacity-40 blur-sm" />
        <div class="absolute inset-0" style="background: linear-gradient(to top, var(--bg-surface), transparent, transparent)"></div>
        ...
        <div class="absolute bottom-4 left-6 right-6 flex items-end justify-between">
          <div>
            ...
            <div class="flex items-center gap-2 mt-0.5">
              <p v-if="developer" class="text-sm" style="color: var(--text-secondary)">by {{ developer }}</p>
              <span v-if="engine" class="text-xs font-bold px-2 py-0.5 rounded-md" style="background: rgba(90,57,104,0.3); color: #b380cc; border: 1px solid rgba(90,57,104,0.4)">{{ engine }}</span>
            </div>
          </div>
          <div class="flex items-center gap-2">
            ...
            <div class="bg-black/60 backdrop-blur-md text-xs font-bold px-2.5 py-1 rounded-lg border border-white/10" style="color: var(--brand)">
      ...
      <div class="p-6 overflow-y-auto space-y-6 flex-1">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="col-span-2">
            ...
            <div class="flex gap-2">
              <input v-model="exePath" type="text" class="modal-input flex-1 font-mono" />
          ...
          <div>
            ...
            <input v-model="version" type="text" placeholder="v1.0" class="modal-input w-full font-mono" style="color: var(--brand)" />
          </div>
          ...
          <div class="col-span-2">
            ...
            <input v-model="f95Url" type="text" placeholder="https://f95zone.to/threads/..." class="modal-input w-full" style="color: var(--brand)" />
          </div>
          
          <div class="col-span-2">
            <label class="modal-label">Command Line Arguments</label>
            <input v-model="commandLineArgs" type="text" placeholder="--fullscreen --no-intro" class="modal-input w-full font-mono" />
          </div>

          <div class="col-span-2 flex items-center justify-between p-3 rounded-lg" style="background: var(--bg-raised); border: 1px solid var(--border)">
            <div>
              <p class="text-sm font-medium" style="color: var(--text-primary)">Run with Japanese Locale</p>
              <p class="text-xs" style="color: var(--text-muted)">Enable this if the game has unreadable text (Mojibake) or crashes due to missing Japanese fonts.</p>
            </div>
            <label class="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" v-model="runJapaneseLocale" class="sr-only peer">
              <div class="toggle-track peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[var(--brand)]"></div>
            </label>
          </div>

          <div class="col-span-2 flex items-center justify-between p-3 rounded-lg" style="background: var(--bg-raised); border: 1px solid var(--border)">
            <div>
              <p class="text-sm font-medium" style="color: var(--text-primary)">Run in Wayland Compatibility Mode</p>
              <p class="text-xs" style="color: var(--text-muted)">Enable this if your system uses Wayland and the game crashes or freezes on launch.</p>
            </div>
            <label class="relative inline-flex items-center cursor-pointer">
          ...
          <div class="col-span-2 flex items-center justify-between p-3 rounded-lg transition-colors"
               :style="ceInstalled ? 'background: var(--bg-raised); border: 1px solid var(--border)' : 'background: var(--bg-inset); border: 1px solid var(--border); opacity: 0.6'">
            <div>
              <p class="text-sm font-medium flex items-center gap-2" style="color: var(--text-primary)">
                Auto-Launch & Inject Cheat Engine
                <span v-if="!ceInstalled" class="text-[10px] uppercase px-2 py-0.5 rounded font-bold" style="background: var(--bg-overlay); color: var(--text-muted); border: 1px solid var(--border)">Not Installed</span>
              </p>
              <p class="text-xs mt-1" style="color: var(--text-muted)">Spawns Cheat Engine inside the same virtual Wine sandbox immediately after the game starts.</p>
            </div>
            <label class="relative inline-flex items-center" :class="ceInstalled ? 'cursor-pointer' : 'cursor-not-allowed'">
              ...
              <div class="toggle-track peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-600"></div>
            </label>
          </div>
          
          <div class="col-span-2">
            <label class="modal-label">Cover Image URL</label>
            <input v-model="coverImage" type="text" placeholder="https://..." class="modal-input w-full" />
          </div>
          
          <div>
            <label class="modal-label">Engine</label>
            <input v-model="engine" type="text" placeholder="Unity, RPGM, Ren'Py..." class="modal-input w-full" style="color: #b380cc" />
          </div>
        </div>
        ...
        <div>
          <h4 class="text-sm font-bold mb-3 flex items-center gap-2" style="color: var(--text-secondary)">
            <svg class="w-4 h-4" style="color: var(--brand)" viewBox="0 0 20 20" fill="currentColor"><path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z"/></svg>
            Your Ratings
          </h4>
          <div class="space-y-3">
            <div v-for="cat in [
              ...
              <span class="text-xs w-28 shrink-0" style="color: var(--text-secondary)">{{ cat.label }}</span>
                ...
                " class="transition-all hover:scale-110">
                  <svg class="w-5 h-5" :class="[
                    star <= (cat.model === 'ratingGraphics' ? ratingGraphics :
                      ...
                      : 'fill-current'
                  ]" :style="star > (cat.model === 'ratingGraphics' ? ratingGraphics : cat.model === 'ratingStory' ? ratingStory : cat.model === 'ratingFappability' ? ratingFappability : ratingGameplay) ? 'color: var(--border)' : ''" viewBox="0 0 20 20">
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-1.5 mb-2">
            <span v-for="tag in tags" :key="tag"
              class="text-xs px-2 py-0.5 rounded-full flex items-center gap-1" style="background: var(--bg-raised); border: 1px solid var(--border); color: var(--text-secondary)">
              {{ tag }}
              <button @click="removeTag(tag)" class="hover:text-red-400 transition-colors ml-0.5" style="color: var(--text-muted)">&times;</button>
            </span>
          </div>
          <div class="flex gap-2">
      ...
      <div v-if="showSavePanel" class="px-6 py-3" style="border-top: 1px solid var(--border); background: var(--bg-inset)">
        <div class="flex items-center justify-between mb-2">
          <h4 class="text-xs font-semibold uppercase tracking-wider" style="color: var(--text-secondary)">Save File Locations</h4>
          <button @click="showSavePanel = false" class="text-xs hover:text-white transition-colors" style="color: var(--text-muted)">✕ Close</button>
        </div>
        <div v-if="searchingSaves" class="text-xs py-2 flex items-center gap-2" style="color: var(--text-secondary)">
          <svg class="w-3 h-3 animate-spin" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 2a10 10 0 0 1 10 10"/></svg>
          Searching...
        </div>
        <div v-else-if="saveResults.length === 0" class="text-xs py-2" style="color: var(--text-muted)">
          No save files found. The game may store saves in an unexpected location.
        </div>
        <div v-else class="space-y-1.5 max-h-32 overflow-y-auto">
          <button v-for="(result, i) in saveResults" :key="i" @click="openSaveFolder(result.path)"
            ...
            <div class="flex-1 min-w-0">
              <p class="text-xs font-medium truncate" style="color: var(--text-primary)">{{ result.description }}</p>
              <p class="text-[10px] truncate font-mono" style="color: var(--text-muted)">{{ result.path }}</p>
            </div>
            <span class="text-[10px] px-1.5 py-0.5 rounded shrink-0" style="background: var(--bg-raised); color: var(--text-muted); border: 1px solid var(--border)">{{ result.type }}</span>
          </button>
        </div>
        <p class="text-[10px] mt-2 italic" style="color: var(--text-muted)">⚠ Auto-detection is best-effort. Some games store saves in non-standard locations.</p>
      </div>

      <!-- Footer Actions -->
      <div class="px-6 py-4 flex items-center justify-between" style="border-top: 1px solid var(--border); background: var(--bg-inset)">
        <div class="flex gap-2">
          ...
          <button v-if="f95Url" @click="openInBrowser"
            class="hover:bg-purple-500/10 px-3 py-2 rounded-lg text-xs font-medium transition-all flex items-center gap-1" style="color: #b380cc">
          ...
          <button @click="findSaves" :disabled="searchingSaves"
